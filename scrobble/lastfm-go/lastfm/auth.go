...
func (api *Api) Login(username, password string) (err error) {
	...
	if err = callPostWithoutSession("auth.getmobilesession", api.uriBase, api.params, args, &result, P{
...
func (api *Api) LoginWithToken(token string) (err error) {
	...
	args := P{"token": token}
	if err = callPostWithoutSession("auth.getsession", api.uriBase, api.params, args, &result, P{"plain": []string{"token"}}); err != nil {
		return
	}
	api.params.sk = result.Key
